...
Fixpoint eval (F : PL) (I:V -> bool) : bool :=
  ...
  | TruePL => true
  | FalsePL => false
  | NotPL A => negb (eval A I)
  | OrPL A B => orb (eval A I) (eval B I)
  | AndPL A B => andb (eval A I) (eval B I)
  end.
...
Proof.
  ...
  unfold equiv_PL in H12.
  unfold equiv_PL in H23.
  unfold equiv_PL.
  intros I.
  rewrite -> H12.
  apply H23.
...
Fixpoint vars (F : PL) : list V :=
  ...
  | FalsePL => []
  | NotPL A => vars A
  | OrPL A B => (vars A) ++ (vars B)
  | AndPL A B => (vars A) ++ (vars B)
  end.
...
Fixpoint nbFE (a:A_2_3) : nat :=
  match a with
  | fe_A_2_3 => 1
  | nb_A_2_3 a1 a2 => nbFE a1 + nbFE a2
  | nt_A_2_3 a1 a2 a3 => nbFE a1 + nbFE a2 + nbFE a3
  end.
...
Fixpoint pas_de_ou {V:Set} (F : @PL V) : Prop :=
  ...
  | TruePL => True
  | FalsePL => True
  | NotPL F1 => pas_de_ou F1
  | OrPL _ _ => False
  | AndPL F1 F2 => and (pas_de_ou F1) (pas_de_ou F2)
  end.
...
Fixpoint supprime_ou {V:Set} (F : @PL V) : @PL V :=
  ...
  | TruePL => TruePL
  | FalsePL => FalsePL
  | NotPL F1 => NotPL (supprime_ou F1)
  | OrPL F1 F2 => NotPL (AndPL (NotPL (supprime_ou F1)) (NotPL (supprime_ou F2)))
  | AndPL F1 F2 => AndPL (supprime_ou F1) (supprime_ou F2)
  end.
