...
forall x , forall y, (comp_alphabet x y = true) -> x=y.
Proof.
Admitted.
...
Definition comp_option_nat (a: option nat)(b: option  nat) : bool :=
match a,b with
  |None,None => true
  |None,Some x => false
  |Some y,None => false
  |Some y, Some x => Nat.eqb x y
end.
...
Definition fsta (toto: Alphabet*Alphabet): Alphabet :=
match toto with
  |(x,y)=> x
end.

Definition snda (toto: Alphabet*Alphabet): Alphabet :=
match toto with
  |(x,y)=> y
end.
...
Definition comp_pair_nat (toto: nat*nat)(tata: nat*nat) : bool:
match toto,tata with
  |(x, y),(v, w) => (Nat.eqb x v) andb (Nat.eqb y w)
end.
