...
Inductive Automate : Type :=
  | AUTO: (list nat)*(list Alphabet)*nat*(list nat)*(nat -> Alphabet -> option nat) -> Automate.
...
Definition M_nb_b_impair : Automate := AUTO([1;2],(a::b::[]),1,[2],delta_nb_b_impair).
...
(* Exemple : définir l'automate à trois états qui accepte les mots commençant et finissant par 'a',
             et donner des tests unitaires. Définir pour cela le graphe puis l'automate qui l'utilise. *)
...
(* Cette preuve utilise la logique du premier ordre, nous reviendrons dessus plus tard. *)
